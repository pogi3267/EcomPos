...
namespace ApplicationWeb.Areas.Admin.Controllers.APIs.Inventory
{
    ...
    [Authorize]
    public class CollectionFinalizedController : ControllerBase
    {
        // GET
        private readonly ICollectionFinalizedService _service;

        public CollectionFinalizedController(ICollectionFinalizedService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> GetInitial()
        {
            try
            {
                var data = await _service.GetInitial();
        ...
        public async Task<IActionResult> SaveAsync([FromForm] Collection model)
        {
            try
            {
                ...
                data.FinalizedDate = model.FinalizedDate;
                data.FinalizedStatus = model.FinalizedStatus;
                data.Approved = model.FinalizedStatus == "pass" ? true : false;
