...
namespace EcomarceOnlineShop.Areas.Admin.Controllers.APIs.SetupAndConfigurations
{
    ...
    {
        private readonly IShippingCountryService _service;

        public ShippingCountryController(IShippingCountryService service)
        {
            _service = service;
        }
        ...
        [HttpGet("Edit/{id}")]
        [Authorize(Policy = "CountryEditPolicy")]
        public async Task<IActionResult> GetCountryAsync(int id)
        {
            try
            {
                Country data = await _service.GetAsync(id);
        ...
        [HttpPost("Save")]
        [Authorize(Policy = "CountryCreatePolicy")]
        public async Task<IActionResult> SaveAsync([FromForm] Country model)
        {
            try
            {
                Country entity = model;
                if (entity.CountriesId > 0)
                {
        ...
        public async Task<IActionResult> DeleteCountryAsync([FromRoute] int id)
        {
            try
            {
                Country entity = await _service.GetAsync(id);
