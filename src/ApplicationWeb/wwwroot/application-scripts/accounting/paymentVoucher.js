(function () {
    ...
    // urls
    const saveUrl = "/api/AccountPaymentVoucher/Save";
    const deleteUrl = "/api/AccountPaymentVoucher/Delete";
    const editUrl = "/api/AccountPaymentVoucher/Edit";
    const getInitialUrl = "/api/AccountPaymentVoucher/GetInitial";
    const getLedgerBalanceById = "/api/AccountPaymentVoucher/GetLedgerBalanceById";
    const getAccountHeadBySupplierId = "/api/AccountPaymentVoucher/GetAccountHeadBySupplierId";
    ...
    const PayToChange = () => {
        ...
        selector.accountHeadId.html("");
        const accountType = selector.accountType.val();
        accountType == 2 && accountType !== '' ? selector.supplierIdDropdown.show()  : selector.payeeToTextbox.show() ;
    }
    ...
    const GetLedgerBalance = async id => {
        try {
            ...
            selector.ledgerBalance.val(response.toFixed(2));
        } catch (e) {
            Failed(e);
        }
    }
    ...
    const GenerateList = () => {
        let columns = [
            ...
            { "data": "subTotal", "name": "subTotal", "autowidth": true, "orderable": true },
            {
                "data": null,
                "render": function (data, type, full, meta) {
                    return ButtonPartial('PaymentVoucher', full.accountVoucherId);
                }
            },
        ];
        let dtLoader = DataTableLoader("/api/AccountPaymentVoucher/list", columns);
