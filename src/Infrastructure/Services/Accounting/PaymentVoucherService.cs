...
namespace Infrastructure.Services.Accounting
{
    public class PaymentVoucherService : IPaymentVoucherService
    {
        ...
        public PaymentVoucherService(IDapperService<AccountVoucher> service) : base()
        {
            _service = service;
        ...
        public async Task<List<AccountVoucher>> GetListAsync(string searchBy, int take, int skip, string sortBy, string sortDir)
        {
            try
            {
                ...
                string sql = $@"SELECT 
                               ...
                               WHERE vou.AccouVoucherTypeAutoID=2 AND vou.IsDeleted = 0
                               ";
        ...
        public async Task<AccountVoucher> GetInitial()
        {
            try
            {
                ...
                var query = $@" 
                                ...
                                SELECT Id = OperationalUserId, Text = OP.Name, Description = OP.Address
        ...
        public async Task<double> GetLedgerBalanceById(int accountLedgerId)
        {
            try
            {
                var query = $@"SELECT (ISNULL(sum(IsNull(DebitAmount,0)),0) - ISNULL(sum(IsNull(CreditAmount,0)),0)) AS Amount 
                            FROM AccountVoucherDetails WHERE ChildId={accountLedgerId}";
                return await _service.GetSingleDoubleFieldAsync(query);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public async Task<List<Select2OptionModel>> GetAccountHeadBySupplierId(int supplierId)
        {
            try
            {
                var query = $@"WITH LEDG AS (
	                                select Id from AccountLedger AL 
	                                Where RelatedId={supplierId}
                                )
                                select Id=AL.Id,Text='['+ParentName+']-'+ Code 
                                from AccountLedger AL 
                                Where RelatedId={supplierId}
                                UNION
                                select Id=AL.Id,Text='['+ParentName+']-'+ Code 
                                from AccountLedger AL 
                                INNER JOIN LEDG ON LEDG.Id = AL.ParentId";
                var queryResult = await _connection.QueryMultipleAsync(query);
        ...
        public string InvoiceGenerate()
        {
            try
            {
                ...
                _connection.Execute("sp_PaymentVoucherInvoice", parameters, commandType: CommandType.StoredProcedure);
        ...
        private async Task<AccountVoucher> GetVoucherByVoucherIdAsync(int VoucherId, SqlTransaction transaction)
        {
            ...
            var query = $@" SELECT AccountVoucherDetailId,
                            ...
                            WHERE AccountVoucherId = {VoucherId}  and TypeId=1";
        ...
        public async Task<int> SaveAsync(AccountVoucher entity)
        {
            try
            {
                ...
                entity.AccountVoucherDetails.ForEach(item =>
                {
                    ...
                    if (item.TypeId != AmountType.CREDIT_AMOUNT)
                    {
                        item.TypeId = AmountType.DEBIT_AMOUNT;
                    }
                    item.IsActive = true;
        ...
        public async Task<AccountVoucher> GetAsync(int id)
        {
            try
            {
                var query = $@"SELECT * FROM AccountVoucher WHERE AccountVoucherId = {id};
                              ...
                              ,CreditAmount,VoucherDate,Reference,IsActive,BranchId,Created_At,Updated_At
                              ,Created_By,Updated_By FROM AccountVoucherDetails WHERE AccountVoucherId = {id}  and TypeId=1;
